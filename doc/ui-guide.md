...
#### 앱 백그라운드/포그라운드
- 백그라운드 전환 시: PlayerData push (기존 동기화 정책 유지)
- 포그라운드 복귀 시:
...
│                         │
├─────────────────────────┤
│  🛡 부적 x2 보유중        │  ← 부적 인디케이터 (보유시만, P2)
├─────────────────────────┤
│  [💰 판매 2,310G] [📦 수집] │  ← 보조 버튼 (강화 위에 배치)
├─────────────────────────┤
│  ┌─────────────────────┐│
│  │    🔨 강 화          ││  ← 메인 버튼 (하단, 크게)
│  │    20% · 600G       ││  ← 확률+비용은 여기에만 표시
│  └─────────────────────┘│
...
|------|------|------|
| 상단 바 | 8% | 골드, 파편 (P2) |
| 검 디스플레이 | 40% | 검 이미지 + 이름 + 테마 + 강화 이펙트 영역 |
| 부적 표시 | 3% | 보호의 부적 보유량 (P2, 보유시만 표시) |
| 액션 영역 | 29% | 판매/수집 (위) + 강화 메인 버튼 (아래, 크게) + 광고 골드 |
| 하단 네비 | 10% | 3탭 네비게이션 |
...
public class EnhanceButton : MonoBehaviour
{
    ...
    public void UpdateInfo(int level, double rate, int cost, int gold)
    {
        _rateText.text = level < 15 ? $"{rate * 100:F0}%" : "???";
...
| +1 ~ +9 | 표시 | 숨김 | 활성 | 표시 |
| +10 ~ +20 | 표시 | 표시 | 활성 | 표시 |
| 골드 부족 | 표시 | 표시 | 비활성(회색) | 강조(펄스) |
| 연출 재생 중 | 비활성 | 비활성 | 비활성 | 비활성 |
| pendingAdProtection | 비활성 | 비활성 | 비활성 | 비활성 |

#### 광고 버튼 표시 규칙

| 버튼 | 위치 | 표시 조건 | 비고 |
|------|------|----------|------|
| 📺 골드 받기 | 강화 버튼 아래 | 항상 표시 (골드 부족 시 강조) | +200골드, 무제한 |
...
**P1 (MVP):** 팝업 없이 인라인 처리. 파괴 연출 후 유저 터치로 나무검 리셋.

```
[파괴 연출 재생] → 파괴 텍스트 ("파괴!" 빨간색)
    → "성공했다면 💰 6,360G 였는데..." 도발 메시지
    → [터치하여 계속] 표시 → 유저 터치 시 나무검 리셋 (ConfirmDestroyCommand dispatch)
```
...
[1.0s] "터치하여 계속" 텍스트 페이드 인 (반복 펄스)
[유저 터치] 나무검 페이드 인 (ConfirmDestroyCommand dispatch)
[+0.2s] 강화 버튼 재활성화
```
...
| 파괴 이펙트 | 빨간 텍스트 + 도발 메시지 + 터치하여 계속 | 파괴 팝업 (광고 보호권 선택) | 파편 파티클 + 사운드 + 진동 |
...
- 파괴 시 인라인 도발 메시지 + [터치하여 계속] → 나무검 리셋 (팝업 없음)
- BasicEnhanceAnimation (최소 연출)
- 광고 버튼 UI 배치 (📺 골드 받기) — 탭 시 "준비 중" 토스트, 실제 연동은 P2
